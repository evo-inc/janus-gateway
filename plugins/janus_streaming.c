...
typedef struct janus_streaming_session {
	...
	volatile gint destroyed;
	janus_refcount ref;
	gint64 last_received_rtcp; /* Used to detect when listeners vanish */
#ifdef HAVE_LIBPULSE
	gboolean talkback;
...
static json_t *janus_streaming_stop_streaming(janus_streaming_session *session) {
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			if(!strcasecmp(type->value, "rtp")) {
				...
				janus_config_item *diface = janus_config_get(config, cat, janus_config_type_item, "dataiface");
				janus_config_item *dbm = janus_config_get(config, cat, janus_config_type_item, "databuffermsg");
				janus_network_address iface_value, data_iface;
				if(file == NULL || file->value == NULL) {
...
#ifndef HAVE_SCTP
				if(dodata) {
					JANUS_LOG(LOG_ERR, "Can't add 'rtsp' stream '%s': no datachannels support......\n", cat->name);
					cl = cl->next;
					continue;
				}
#endif
				if(dodata && diface) {
					...
					if(janus_network_lookup_interface(ifas, diface->value, &data_iface) != 0) {
						JANUS_LOG(LOG_ERR, "Can't add 'rtsp' stream '%s', invalid network interface configuration for data...\n", cat->name);
						cl = cl->next;
						continue;
					}
				}
...
static json_t *janus_streaming_process_synchronous_request(janus_streaming_session *session, json_t *message) {
	...
	if(!strcasecmp(request_text, "list")) {
		...
		if(!strcasecmp(type_text, "rtp")) {
			...
			/* RTSP source*/
			janus_network_address multicast_iface, data_iface;
			json_t *name = json_object_get(root, "name");
			json_t *desc = json_object_get(root, "description");
			json_t *md = json_object_get(root, "metadata");
			json_t *is_private = json_object_get(root, "is_private");
			...
			json_t *data = json_object_get(root, "data");
			json_t *dport = json_object_get(root, "dataport");
			json_t *diface = json_object_get(root, "dataiface");
			json_t *dbm = json_object_get(root, "databuffermsg");
			gboolean doaudio = audio ? json_is_true(audio) : FALSE;
			gboolean dovideo = video ? json_is_true(video) : FALSE;
			gboolean dodata = data ? json_is_true(data) : FALSE;
			gboolean buffermsg = data && dbm ? json_is_true(dbm) : FALSE;
			...
			if(!doaudio && !dovideo) {
				...
				if (dodata && diface) {
					if(janus_network_lookup_interface(ifas, (const char *)json_string_value(diface), &data_iface) != 0) {
						JANUS_LOG(LOG_ERR, "Can't add 'rtsp' stream '%s', invalid network interface configuration for data...\n", (const char *)json_string_value(name));
						error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
						g_snprintf(error_cause, 512, ifas ? "Invalid network interface configuration for stream" : "Unable to query network device information for data");
						janus_mutex_lock(&mountpoints_mutex);
						g_hash_table_remove(mountpoints_temp, string_ids ? (gpointer)mpid_str : (gpointer)&mpid);
						janus_mutex_unlock(&mountpoints_mutex);
						goto prepare_response;
			...
			mp = janus_streaming_create_rtsp_source(
					...
					dodata,
					dodata && diface ? &data_iface : NULL,
					dport ? json_integer_value(dport) : 0,
					buffermsg,
					error_on_failure);
			janus_mutex_lock(&mountpoints_mutex);
			g_hash_table_remove(mountpoints_temp, string_ids ? (gpointer)mpid_str : (gpointer)&mpid);
...
void janus_streaming_incoming_data(janus_plugin_session *handle, janus_plugin_data *packet) {
	...
	uint16_t len = packet->length;
	if((buf == NULL) || (len == 0)) {
		JANUS_LOG(LOG_ERR, "Incoming data doesn't have a valid buffer\n");
		return;
	}
#ifdef HAVE_LIBPULSE
	if (len > 3) {
		...
		if (cmd[0] == 'C' && cmd[1] == 'M' && cmd[2] == 'D') {
			char *text = g_malloc(dLen  + 1);
			memcpy(text, data, dLen);
			*(text+dLen) = '\0';
			JANUS_LOG(LOG_VERB, "Got a DataChannel command (%zu bytes): %s\n", strlen(text), text);
			if (strncmp(text, "start_talking", dLen) == 0) {
				janus_streaming_start_talkback(session);
			} else if (strncmp(text, "stop_talking", dLen) == 0) {
...
static void *janus_streaming_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
      ...
      /*
			...
			JANUS_LOG(LOG_VERB, "User stop\n");
			result = janus_streaming_stop_streaming(session);
		} else {
			JANUS_LOG(LOG_VERB, "Unknown request '%s'\n", request_text);
			error_code = JANUS_STREAMING_ERROR_INVALID_REQUEST;
			g_snprintf(error_cause, 512, "Unknown request '%s'", request_text);
...
janus_streaming_mountpoint *janus_streaming_create_rtsp_source(
	...
	JANUS_LOG(LOG_VERB, "Audio %s, Video %s, Data %s\n", doaudio ? "enabled" : "NOT enabled", dovideo ? "enabled" : "NOT enabled", dodata ? "enabled" : "NOT enabled");
...
#ifdef HAVE_SCTP
		data_fd = janus_streaming_create_fd(dport, INADDR_ANY, diface,
			NULL, 0, "Data", "data", name ? name : tempname, FALSE);
		if(data_fd < 0) {
			JANUS_LOG(LOG_ERR, "Can't bind to port %d for data...\n", dport);
			janus_mutex_unlock(&mountpoints_mutex);
			return NULL;
	...
	live_rtsp_source->data_fd = data_fd;
	live_rtsp_source->data_port = dodata ? dport : -1;
	live_rtsp_source->data_iface = dodata && !janus_network_address_is_null(diface) ? *diface : nil;
...
static void janus_streaming_relay_rtp_packet(gpointer data, gpointer user_data) {
	...
	if (now - session->last_received_rtcp > 20000000L) {
		JANUS_LOG(LOG_INFO, "Stopping due to 20 sec since RTCP received\n");
		janus_streaming_send_event(session, janus_streaming_stop_streaming(session));
	}

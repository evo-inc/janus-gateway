...
static json_t * janus_streaming_stop(janus_streaming_session* session, const char* reason);
...
static void *janus_streaming_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
      ...
      /*
			...
			result = janus_streaming_stop(session, "User Request");
		} else {
			JANUS_LOG(LOG_VERB, "Unknown request '%s'\n", request_text);
			error_code = JANUS_STREAMING_ERROR_INVALID_REQUEST;
			g_snprintf(error_cause, 512, "Unknown request '%s'", request_text);
...
janus_streaming_mountpoint *janus_streaming_create_rtsp_source(
	...
	live_rtsp->codecs.video_codec = JANUS_VIDEOCODEC_H264;
	...
	if(packet->is_rtp) {
		...
		if (session->last_relayed == 0) {
			...
			if (now - session->last_received_rtcp >= 60000000L) {
				janus_streaming_send_event(session, janus_streaming_stop(session, "RTCP Timeout 60s"));
			}
		} else {
			// Close session if we had not received RTCP packet back in 20s
			if (now - session->last_received_rtcp >= 20000000L) {
				janus_streaming_send_event(session, janus_streaming_stop(session, "RTCP Timeout 20s"));
			}
		}
